...
- 用户反馈问题、报告错误、描述异常时（如"看不到"、"不行"、"出错"、"找不到"、"没反应"），这是对上一轮结果的反馈，不是新任务。你应该：
  1. 先理解用户反馈的具体问题是什么
  2. 分析可能的原因
  3. 针对性地解决问题或给出建议
  4. 绝对不要忽略反馈去重新执行一个新任务
...
- 生成 HTML 页面、产品页、展示页、图表、流程图时，必须使用 create_canvas 工具渲染到 Canvas 面板，不要在聊天框直接输出 HTML 代码
  ...
  - 引用本地图片时，使用 /api/local-file?path=<绝对路径> 作为 img src（例如: <img src="/api/local-file?path=~/Desktop/photo.jpg">），不要用 file:// 协议或 base64
  - 引用 workspace 内文件时，使用 /api/workspace/files/<相对路径> 作为 src
- Canvas 导出: 用户要求导出 Canvas 为 PDF/PNG/HTML 文件时，必须使用 export_canvas 工具，不要用 run_terminal 手动操作
  ...
  - 再用 export_canvas 导出，文件会保存到 ~/.xjd-agent/workspace/exports/ 目录
  - 导出完成后，用 run_terminal 执行 `open ~/.xjd-agent/workspace/exports/` 帮用户打开导出目录
...
class AgentEngine:
    ...
    async def run_turn(
        ...
        # 反馈检测: 用户在反馈问题时，跳过技能注入，避免答非所问
        from agent.tools.tool_selector import _FEEDBACK_PATTERNS
        _is_feedback = any(pat in user_message.lower() for pat in _FEEDBACK_PATTERNS)
        if _is_feedback and injected:
            injected.skill_message = ""
            injected.matched_skill_id = ""
            logger.debug("Feedback detected, skipping skill injection")
        ...
        while round_idx < max_safety_rounds:
            ...
            if not response.tool_calls:
                ...
                # Learn cache — 技能执行成功后才缓存匹配
                if (
                    self._active_skill
                    and self._skill_manager
                    and turn_result.content
                    and not any(
                        kw in turn_result.content.lower()
                        for kw in ("error:", "failed:", "错误:", "失败:")
                    )
                ):
                    self._skill_manager.record_match(
                        user_message, self._active_skill.skill_id,
                    )
